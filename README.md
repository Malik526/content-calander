...
Public routes: `/` (homepage), `/privacy`, `/terms` — real, stable public pages needed for TikTok Developer Portal configuration. **Milestone 3.5** added a real product app shell alongside them: `/app`, `/app/library`, `/app/queue`, `/app/settings` — mobile-first, with mobile bottom nav / desktop side nav. **Milestone 3.6** replaced that shell's mock session with real authentication: `/login` and `/auth/callback` (new), backed by real Supabase Auth (Google) — `NEXT_PUBLIC_ALLOW_MOCK_SESSION` no longer exists anywhere; a production build with Supabase unconfigured now fails closed unconditionally. `/app/settings` now shows a real, backend-verified TikTok connection status (see below), not mocked data. Deployed via the repo-root `netlify.toml` (`base = "web"`, static export, no server-rendered routes yet). Design tokens (colors/fonts/status colors) are centralized in `web/app/globals.css`; site text/contact details in `web/lib/site-config.ts` — see `web/README.md` for the full breakdown, including the **placeholder contact email that must be replaced before deploying publicly**.

**Backend API (Milestone 3.6):** `src/content_automation/api/` — a real FastAPI app (`GET /api/me`, `/api/platforms/tiktok/{status,connect,callback,disconnect}`), run via `python3 cli/run_api.py`. Every protected route verifies a real Supabase Auth bearer token server-side (`identity/token_verification.py`) — never a client-supplied `user_id`. The hosted TikTok OAuth connection flow reuses `publishing/tiktok/auth.py`'s proven PKCE/token logic unmodified; a new `platform_credentials` table (Fernet-encrypted, `config.CREDENTIAL_ENCRYPTION_KEY`) holds a hosted connection's token pair, separate from the local CLI's own token file, which remains completely untouched. See `docs/decisions/0011-real-authentication-and-tiktok-connection.md`.

No batch upload, real scheduling controls, real queue/calendar backend data, hosted workers, billing, or analytics exist yet — see `docs/decisions/0010-frontend-app-shell.md`/`0011-real-authentication-and-tiktok-connection.md` for the architecture decisions and `docs/architecture/hosted-product-boundary.md` §18 for how the frontend and backend boundaries line up.
