...
- **Milestone 3.6 (real authentication + TikTok account connection) is PARTIAL** — see `docs/evaluations/productization/milestone-3.6-auth-tiktok-connection.md` and `docs/decisions/0011-real-authentication-and-tiktok-connection.md`. Everything buildable without real external credentials is complete and tested: real Supabase Auth (Google) replaces the Milestone 3.5 mock session — `NEXT_PUBLIC_ALLOW_MOCK_SESSION` no longer exists anywhere; a real FastAPI backend (`src/content_automation/api/` — `GET /api/me`, `/api/platforms/tiktok/{status,connect,callback,disconnect}`) verifies every request via `identity/token_verification.py`, never a client-supplied `user_id`; a hosted TikTok OAuth connection flow reuses `publishing/tiktok/auth.py`'s PKCE/token logic unmodified, with a new encrypted `platform_credentials` table (Fernet, CAS-refreshed) alongside — not replacing — the local CLI's own token file/`fcntl` lock; `cli/link_bootstrap_user.py` exists (dry-run-capable, tested) to link the pre-3.6 bootstrap user's real data to a real login, but has **not been run against real data yet**. **Still open, blocking COMPLETE status:** the user's own real Google Cloud OAuth client / Supabase Auth configuration was still being set up as of this writing — `NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_ANON_KEY`/`NEXT_PUBLIC_API_BASE_URL` are not yet set in `netlify.toml` (deployed shell correctly fails closed until they are), no real TikTok OAuth connection has been completed, and the two new tables (`platform_credentials`/`oauth_states`) have not been exercised against real Postgres. Do not treat 3.6 as closed or start un-blocking 3.7 (batch upload) on the assumption real auth is live in production — verify against `CHANGELOG.md`/the evaluation record first.
- **Next up: finish Milestone 3.6's real-credential validation, then Milestone 3.7+.** As of this writing: no hosted scheduler/worker, no hosted-worker-safe distributed lock for TikTok credential refresh (a real limitation, not an oversight — see ADR-0011 "Consequences"), no media deletion/retention policy implemented (deliberately deferred — see ADR-0009), no batch upload, no real scheduling controls, no real queue/calendar data wired to a backend, no real API hosting provider selected. Do not introduce any of the still-missing pieces, and do not change existing scheduling/publishing/retry/reconciliation/auth behavior, unless the task you were given explicitly asks for it. Check `CHANGELOG.md`'s most recent entries and `PROJECT_STATE.md` before assuming what "current" means — both change often.
