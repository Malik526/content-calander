...
- **Next up: Milestone 3.5+.** As of this writing: no FastAPI (or other web framework) app, no real authentication provider integration, no hosted scheduler/worker, no hosted credential storage, no media deletion/retention policy implemented (deliberately deferred — see ADR-0009). Do not introduce any of the still-missing pieces, and do not change existing scheduling/publishing/retry/reconciliation/auth behavior, unless the task you were given explicitly asks for it. Check `CHANGELOG.md`'s most recent entries and `PROJECT_STATE.md` before assuming what "current" means — both change often.
