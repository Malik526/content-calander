...
- **Milestone 3.6 (real authentication + TikTok account connection) is PARTIAL** — see `docs/evaluations/productization/milestone-3.6-auth-tiktok-connection.md` and `docs/decisions/0011-real-authentication-and-tiktok-connection.md`. Everything buildable without real external credentials is complete and tested: real Supabase Auth (Google) replaces the Milestone 3.5 mock session — `NEXT_PUBLIC_ALLOW_MOCK_SESSION` no longer exists anywhere; a real FastAPI backend (`src/content_automation/api/` — `GET /api/me`, `/api/platforms/tiktok/{status,connect,callback,disconnect}`) verifies every request via `identity/token_verification.py`, never a client-supplied `user_id`; a hosted TikTok OAuth connection flow reuses `publishing/tiktok/auth.py`'s PKCE/token logic unmodified, with a new encrypted `platform_credentials` table (Fernet, CAS-refreshed) alongside — not replacing — the local CLI's own token file/`fcntl` lock; `cli/link_bootstrap_user.py` exists (dry-run-capable, tested) to link the pre-3.6 bootstrap user's real data to a real login — **run against real data as of 2026-09-21** (verified directly against real Postgres: one real `auth_identities` row, all 5 pre-existing videos linked to the real user). A real Supabase login has also been completed. A 2026-09-21 fix (see `CHANGELOG.md`) replaced the hosted TikTok connect flow's request-derived `redirect_uri` with an explicitly configured `config.TIKTOK_WEB_REDIRECT_URI`, since the former could never work behind a real reverse-proxy deployment. **Still open, blocking COMPLETE status:** no real TikTok OAuth connection has been completed yet (the redirect-uri fix above was found while attempting it — not yet re-attempted since), and the two new tables (`platform_credentials`/`oauth_states`) have not been exercised against real Postgres. Do not treat 3.6 as closed on the assumption the TikTok connection or those two tables are live — verify against `CHANGELOG.md`/the evaluation record first.
- **Milestone 3.6.1 (production API deployment) is IN PROGRESS.** Railway is the selected API host (`railway.json` at repo root has the build/start/healthcheck config — Nixpacks build running `pip install -r requirements.txt && pip install -e .`, start command `python3 cli/run_api.py --host 0.0.0.0 --port $PORT`, healthcheck `/api/health`); the frontend is on Netlify's free subdomain (`picklebatch.netlify.app`, no custom domain owned yet). The repo is deploy-ready but **not yet actually deployed** — the Railway project/service creation, production environment variables, and the resulting permanent `TIKTOK_WEB_REDIRECT_URI`/`NEXT_PUBLIC_API_BASE_URL` values are pending the user's own dashboard/account work (see `CHANGELOG.md` for whether this has since been completed and verified — do not assume it's live from this note alone).
- **Next up: finish Milestone 3.6.1's live deployment validation, then real TikTok OAuth, then Milestone 3.7+.** As of this writing: no hosted scheduler/worker, no hosted-worker-safe distributed lock for TikTok credential refresh (a real limitation, not an oversight — see ADR-0011 "Consequences"), no media deletion/retention policy implemented (deliberately deferred — see ADR-0009), no batch upload, no real scheduling controls, no real queue/calendar data wired to a backend. Do not introduce any of the still-missing pieces, and do not change existing scheduling/publishing/retry/reconciliation/auth behavior, unless the task you were given explicitly asks for it. Check `CHANGELOG.md`'s most recent entries and `PROJECT_STATE.md` before assuming what "current" means — both change often.
