...
- **Milestone 3.5 (mobile-first responsive web app shell) is complete** — see `docs/evaluations/productization/milestone-3.5-mobile-web-shell.md` and `docs/decisions/0010-frontend-app-shell.md`. `web/` now has a real product app shell (`/app`, `/app/library`, `/app/queue`, `/app/settings`) alongside the pre-existing public marketing site (`/`, `/privacy`, `/terms`), a route-structural public/product boundary, an API-client boundary (`web/lib/api/client.ts`) nothing calls yet (no backend exists), and a session boundary (`web/lib/session.tsx`) backed only by a fail-closed-by-default development mock user. **The Netlify deployment currently sets `NEXT_PUBLIC_ALLOW_MOCK_SESSION=true`** (`netlify.toml`) as a deliberate, temporary shell-development flag — not itself a secret, but see ADR-0010's "BLOCKER BEFORE REAL USER DATA ACCESS" before any future milestone lets `/app` touch real user-owned data or call an authenticated API; that checklist (real auth, remove the mock path, remove the Netlify flag, re-verify fail-closed, never trust a client-supplied `user_id`) must be satisfied first.
- **Next up: Milestone 3.6+.** As of this writing: no FastAPI (or other web framework) app, no real authentication provider integration (frontend or backend), no hosted scheduler/worker, no hosted credential storage, no media deletion/retention policy implemented (deliberately deferred — see ADR-0009), no TikTok connection UI, no batch upload, no real scheduling controls, no real queue/calendar data wired to a backend. Do not introduce any of the still-missing pieces, and do not change existing scheduling/publishing/retry/reconciliation/auth behavior, unless the task you were given explicitly asks for it. Check `CHANGELOG.md`'s most recent entries and `PROJECT_STATE.md` before assuming what "current" means — both change often.
...
- `web/` (frontend, Milestone 3.5+): `npm run test` (Vitest + React Testing Library) — current baseline: **46 passing**, `npm run lint` (ESLint) and `npm run build` (production static export, which also runs the TypeScript typecheck) must both be clean. Run these from inside `web/`, independently of the Python suite above — the two have no shared tooling or code. `tests/lib/no-secrets-in-client-bundle.test.ts` and `tests/lib/session.test.tsx` are guardrail tests, not ordinary coverage — do not delete or weaken them; see ADR-0010's "Authentication Boundary" for what they enforce.
...
- `.env` — `TIKTOK_CLIENT_KEY`/`TIKTOK_CLIENT_SECRET`, `ANTHROPIC_API_KEY`, `DATABASE_URL` (Milestone 3.3 — real Postgres connection string with credentials), `SERVICE_ROLE_KEY` (Milestone 3.4 — Supabase service-role key, full storage access, server-side only; note the variable name has no `SUPABASE_` prefix despite `SUPABASE_URL` having one). Never commit, never echo into logs or docs. `DATABASE_URL`/`SERVICE_ROLE_KEY` specifically: never print them, even partially, in terminal output or a file — if you need to confirm which host/project a value points to, parse and print only that portion.
...
- Real authentication provider integration, TikTok connection UI, batch upload, real scheduling controls, real queue/calendar backend data, or hosted workers inside `web/` — the app shell (Milestone 3.5) is a static, mock-session-only surface; see ADR-0010's "BLOCKER BEFORE REAL USER DATA ACCESS" before wiring any real user-owned data or authenticated API call into it.
