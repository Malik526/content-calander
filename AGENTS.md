...
- **Milestone 3.7 (Batch Upload UX) readiness + data-quality/instrumentation follow-up work is done; the milestone itself is IN PROGRESS** — see `docs/evaluations/productization/milestone-3.7-batch-upload-readiness.md` (2026-09-26 addendum). `POST /api/videos` (real multipart batch upload — one `VideoUploadResult` per file, a bad file never aborts the rest of the batch) and `GET /api/videos` (the caller's own videos, newest first) exist (`api/routes/videos.py`); `media.media_storage.create_video_from_upload()` creates a brand-new owned `videos` row directly from freshly-received bytes and uploads it via the already-existing `StorageProtocol`. `web/app/app/library/page.tsx` is a real client component (upload form + real listing). `requirements.txt` has `python-multipart`. **A real hosted upload surfaced a real gap, since corrected:** rows were recording `storage_provider="local"` — investigated and confirmed **not a code bug** (every write site reads `storage.provider_name` dynamically, proven by a stub-backend test) but a missing Railway env var: `config.STORAGE_BACKEND` defaults to `"local"` when unset, and `build_storage()` had never been called from any live entry point before this milestone — if Railway's dashboard was never given `STORAGE_BACKEND=supabase` explicitly, uploaded bytes are sitting in that process's local filesystem, not the real Supabase bucket, regardless of how Supabase-shaped the recorded `storage_key` looks (see ADR-0009's 2026-09-26 addendum for the full record). `api/app.py`'s startup diagnostic now also logs `STORAGE_BACKEND` so this is visible in Railway's logs immediately. `canonical_media_path` staying `NULL` for hosted uploads is confirmed intentional (`storage_key`/`storage_provider` is the one canonical hosted-media identifier). Added minimal event/attempt-level upload-performance instrumentation — `upload_batches`/`upload_attempts` tables (both backends, `postgres_migrations/0004_add_upload_telemetry.sql`) — recording per-file/per-batch server-side timing (`time.monotonic()` deltas, explicitly documented as *not* network latency) and structured `error_code`s; not exposed through any API/UI yet, persistence only. Documented (not built) exactly which media-metadata fields a future background job could populate via `ffprobe`. Implemented navigation-safe uploads: `web/lib/uploads.tsx` (`UploadProvider`/`useUploadManager()`), mounted in `app/app/layout.tsx` above page content like `SessionProvider`, so in-progress upload state survives the user navigating to Queue/Settings and back. **Still open, blocking COMPLETE status:** `STORAGE_BACKEND=supabase` needs confirming/setting on Railway's dashboard and redeploying, and no real hosted upload has validated the corrected metadata or the new telemetry tables yet — see the evaluation doc's addendum for exact next steps. Do not treat 3.7 as closed on the assumption any of this has been validated live — verify against `CHANGELOG.md`/the evaluation record first, exactly like 3.6's own pattern.
- **Milestone 3.7 also now has a Delete Video action** — `DELETE /api/videos/{id}` (`media.media_storage.delete_video`), a small authenticated Library delete with inline confirm in the UI. Refuses (409) rather than cascading if the video is still assigned to a content_slot or has any `platform_posts` row; deletes the stored object (via `StorageProtocol`, skipped for a legacy local-only video) and the `videos` row otherwise, nulling out (not deleting) any `upload_attempts.video_id` reference to preserve that telemetry. No queue/calendar-editing UI exists yet to resolve a 409 from the frontend — that stays a backend/CLI-side-only fix for now. See `CHANGELOG.md`'s "Milestone 3.7 — Delete Video" entry.
- **`videos.file_hash` is no longer globally unique** (Milestone 3.7 re-upload-architecture follow-up) — `videos.id` is a record's real identity; `file_hash` is a reusable content fingerprint. `create_video_from_upload` always creates a new row per upload call now, including a re-upload of byte-identical content (same user or a different one) — there is no more `DuplicateVideoContentError`/idempotent-by-content behavior; that was a deliberate reversal of Milestone 3.7's original design, not a bug fix. Dropped on both backends (SQLite: a structural rebuild migration, `_migrate_videos_drop_file_hash_uniqueness`; Postgres: `postgres_migrations/0005_drop_videos_file_hash_uniqueness.sql`), replaced with a plain index. Full record, including one narrower pre-existing edge case this slightly widens (a local-CLI/hosted-upload cross-tenant hash collision — local ingestion is unaffected in the normal case) and explicitly does not fix, in ADR-0009's newest addendum.
- **A temporary evaluation workflow exists to compare real hosted-upload behavior before/after the change above** — `docs/evaluations/productization/milestone-3.7-upload-benchmark-snapshot.md` (a documented read-only SQL workflow, not a script or new skill/agent — deliberately not over-automated for a one-off comparison). **Its Iteration 1 baseline is not yet captured** — this agent cannot read the real hosted Postgres database directly (sandboxed from production reads); real values still need to be captured (by running that doc's queries against the real database) before the three existing real test videos are deleted, or the comparison this evaluation exists for is lost.
- **Next up: capture Iteration 1 of the upload-benchmark snapshot above (before deleting the three real test videos), then Iteration 2 after re-uploading them; finish Milestone 3.7's live upload validation with real video files (including confirming the Railway `STORAGE_BACKEND` fix); then whatever Batch Upload UX polish that reveals is needed.** As of this writing: no hosted scheduler/worker, no hosted-worker-safe distributed lock for TikTok credential refresh (a real limitation, not an oversight — see ADR-0011 "Consequences"), no *retention* policy (auto-expiry/cleanup) beyond the manual Delete Video action, no ffprobe/duration/dimensions enrichment of uploaded videos (documented roadmap only — see `media/media_storage.py`), no API/UI surface for the upload telemetry tables, no real scheduling controls, no real queue/calendar data wired to a backend (so a video blocked from deletion by a schedule/queue reference cannot yet be un-blocked from the UI). Do not introduce any of the still-missing pieces, and do not change existing scheduling/publishing/retry/reconciliation/auth behavior, unless the task you were given explicitly asks for it. Check `CHANGELOG.md`'s most recent entries and `PROJECT_STATE.md` before assuming what "current" means — both change often.
