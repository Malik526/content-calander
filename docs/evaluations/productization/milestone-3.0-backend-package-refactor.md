...
`README.md`: every `python3 <script>.py` command updated to its real new invocation path (`cli/...` or `tools/evaluation/...`), plus a new "Project Structure" section giving the package/CLI/tooling layout at a glance. `PROJECT_STATE.md`: the same command-path updates. `docs/decisions/` and `docs/evaluations/` (other than this record) were deliberately **not** rewritten — they are historical evidence, accurate at the time they were written; rewriting them to reference paths that didn't exist yet would misrepresent when those milestones actually ran, exactly the brief's own instruction.
